...
// --- GLOBAL CONSTANTS ---
const STUDENT_PERMISSION = "commenter";
...
function showSidebar() {
 ...
 SpreadApp.getUi().showSidebar(html);
}
...
function createMissingDocs() {
 ...
 bodyStyle[DocumentApp.Attribute.BOLD] = false; // <-- THE FIX: Explicitly set bold to false
 ...
 // --- REMOVED: Italic attribute ---
 emailStyle[DocumentApp.Attribute.FOREGROUND_COLOR] = "#777777"; // Lighter gray
 emailStyle[DocumentApp.Attribute.LINE_SPACING] = 1.5; // <-- ADDED
 ...
 questionTitleStyle[DocumentApp.Attribute.LINE_SPACING] = 1.5; // <-- ADDED
     ...
     headers.forEach((title, colIndex) => {
       ...
       if (answer && String(answer).trim() !== "") {
         ...
         if (cleanTitle.trim() !== "") {
           ...
           // This replaces the old try/catch block
     ...
     if (teachers.length > 0 && teachers[0] !== "") {
       teachers.forEach(email => {
         try {
           ...
           Drive.Permissions.create(permission, docId, {
 ...
 for (let i = 1; i < data.length; i++) {
   ...
   if (docUrl && !shareStatus) {
     try {
       ...
       const permission = {
         'role': STUDENT_PERMISSION, // Using your global constant
         'type': 'user',
     ...
     {
       // --- IMPROVED LOGGING ---
       // The error 'e' will now contain the problematic URL if parsing failed
